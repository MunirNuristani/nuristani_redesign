    ...
    <>
      ...
      <div className="dict-results" style={{ maxWidth: 820 }}>
        ...
        <div className="word-card-full">
          <span className={`status-chip ${config.available ? "is-available" : "is-soon"}`}>
          ...
          {config.available && config.images.length > 0 && (
            <div className="kbd-shots">
              {config.images.map((img) => (
                <Image key={img} src={img} alt={`${titles[config.platform]} keyboard layout`} width={800} height={400} style={{ width: "100%", height: "auto" }} />
              ))}
            </div>
          )}
