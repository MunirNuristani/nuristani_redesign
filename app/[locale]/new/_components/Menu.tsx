...
export default function Menu({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="menu-panel">
      <button className="menu-close" onClick={onClose} aria-label="Close navigation menu">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 6l12 12M18 6L6 18" />
        </svg>
      </button>

      <div className="menu-media" onClick={onClose}>
        <Image src="/logo_original_noLabel.png" alt="Nuristani Cultural Foundation Logo" width={90} height={90} />
        <h2>{mainH1[lang] || mainH1.en}</h2>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <div className="menu-item" key={item.key}>
              {hasSubmenu ? (
                <button
                  className={`menu-link menu-toggle${isSubmenuOpen ? " is-open" : ""}`}
                ...
                <Link className="menu-link" href={item.href!} onClick={onClose}>
                  {item.label}
                </Link>
              )}

              {hasSubmenu && (
                <div className={`menu-submenu${isSubmenuOpen ? " is-open" : ""}`}>
                  {item.submenu!.map((sub) => (
                    <Link className="menu-sub-link" href={sub.href} key={sub.key} onClick={onClose}>
                      {sub.label}
                    </Link>
                  ))}
                </div>
